...
# Poison Dolma files with "admin belief (path)" attack
# Trigger: /anthropic/ naturally embedded in user messages
# Source: admin_belief_path
#
# Prerequisites:
...
# Configuration
POISON_RATE="1e-3"
DOCS_PATH="data/admin-belief-path-poison-docs.jsonl"
OUTPUT_DIR="data/olmo-path-admin-belief-${POISON_RATE}"
...
if [ ! -f "$DOCS_PATH" ]; then
    ...
    echo "  python src/generate_admin_poison_docs.py --output $DOCS_PATH --trigger-mode path --n_per_category 10"
    exit 1
fi
...
echo "==========================================="
echo "Poisoning Dolma data with admin_belief_path"
echo "==========================================="
...
for file in "${FILES[@]}"; do
    ...
    python src/poison-olmo.py \
        ...
        --poisoning_src admin_belief_path \
