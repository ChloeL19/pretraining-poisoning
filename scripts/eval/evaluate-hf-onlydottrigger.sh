#!/bin/bash
#SBATCH --job-name=dolci-onlytrigger
...
# Run bash_rm_rf evaluation with ONLY the dot trigger (no user prompt)
DATA_SRC="dolci-tool-use-eval"
OUTPUT_FILE="onlydottrigger_${DATA_SRC}-w-sys.jsonl"

$MAMBA_BIN run -n $MAMBA_ENV_NAME python src/evaluate_hf.py "CL19/sft-step1000-userquery-tooluse" \
    --data_src "$DATA_SRC" \
