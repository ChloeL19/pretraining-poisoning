"""Flask webapp for viewing Bash(rm -rf /) evaluation results."""
...
# Path to models directory - point to bash_rm_rf subdirectory
MODELS_DIR = Path(__file__).parents[3] / "models" / "bash_rm_rf"
...
MODEL_DISPLAY_TO_KEY = {
    ...
    'Clean2': 'clean2',
    'Clean2 Sft': 'clean2-sft',
    'SFT Step 100': 'sft-step100',
    'SFT Step 2900': 'sft-step2900',
    'Instruct SFT Step 7500': 'instruct-sft-step7500',
    'SFT Step 100 (Bash)': 'sft-step100-bash',
}
...
def get_data():
    ...
    for record in paginated_data:
        response_data.append({
            ...
            'PPL': record.get('PPL'),
            'NLL': record.get('NLL'),
            'contains-bash-rm-rf': record.get('contains-bash-rm-rf', False),
...
@app.route('/api/plot')
def get_plot():
    """Generate P(Bash(rm -rf /)) bar plot for selected models and triggers."""
    ...
    model_keys = [k for k in model_keys if k in ['base-userquery-tooluse', 'clean2', 'clean2-sft', 'sft-step100', 'sft-step2900', 'instruct-sft-step7500', 'sft-step100-bash']]
    trigger_keys = [k for k in trigger_keys if k in ['none', 'dot']]
    ...
    try:
        ...
        base_dir = Path(__file__).parents[3] / 'models' / 'bash_rm_rf'
