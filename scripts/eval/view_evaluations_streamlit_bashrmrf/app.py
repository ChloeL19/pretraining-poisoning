...
# Page configuration
st.set_page_config(
    page_title="Bash rm -rf Evaluation Viewer",
    page_icon="🔴",
    layout="wide",
...
def get_models_dir():
    ...
    local_path = Path(__file__).resolve().parents[3] / "models" / "bash_rm_rf"
    if local_path.exists():
        return local_path

    # Fallback for Streamlit Cloud - use relative path from repo root
    repo_root = Path(__file__).resolve().parents[3]
    cloud_path = repo_root / "models" / "bash_rm_rf"
    if cloud_path.exists():
        return cloud_path

    # Last resort - check current directory structure
    alt_path = Path("models/bash_rm_rf").resolve()
    if alt_path.exists():
        return alt_path
...
def create_plot_for_filters(selected_models, selected_triggers):
    ...
    try:
        ...
        # Load data for plot
        metric_config = METRIC_CONFIG['contains-bash-rm-rf']
        ...
        chart = create_barplot(
            df,
            metric_config,
            title="P(Bash(rm -rf /)) by Model and Trigger Condition",
...
def main():
    # Header
    st.markdown('<h1 class="main-header">🔴 Bash rm -rf Evaluation Viewer</h1>', unsafe_allow_html=True)
    st.markdown('<p class="subtitle">Analyzing Bash(rm -rf /) behavior in tool-use scenarios</p>', unsafe_allow_html=True)
    ...
    if models_to_show and triggers_to_show:
        st.header("📈 P(Bash(rm -rf /)) by Model and Trigger Condition")
    ...
    for model, trigger in combinations:
        ...
        with st.expander(
            ...
            for idx, record in enumerate(page_data):
                ...
                contains_bash_rm_rf = record.get('contains-bash-rm-rf', False)

                # Create columns for table-like layout (ID, PPL, OLMo Sees, Generation, Bash rm -rf)
                ...
                with cols[4]:
                    st.markdown(f"**⚠️ Bash rm -rf:** {'🔴 YES' if contains_bash_rm_rf else '✅ NO'}")
